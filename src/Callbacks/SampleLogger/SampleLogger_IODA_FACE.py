...
class SampleLogger_Face_IODA(Callback):
	def __init__(
		self,
		...
		super(SampleLogger_Face_IODA, self).__init__()
	...
	def _log_samples(self, samples: torch.Tensor, model, prefix: str):
		x, y = samples
		x = x.to(model.device)
		y = y.to(model.device)
		...
		caption = '(x, y->y), (xx, y->y), (x, y->yy), (x, y->xy)'

		# but back x in the usual image format
		x  = x.permute([0, 2, 3, 1])
		xx = pred.xx.permute([0, 2, 3, 1])

		# put back the annotation in there real space relatively to the input image
		# the bigger index possible is actually 63
		y  = (y       * 63).int().reshape([-1, 68, 2])
		yy = (pred.yy * 63).int().reshape([-1, 68, 2])
		xy = (pred.xy * 63).int().reshape([-1, 68, 2])

		# create an image with a line draw btw the GT and the pred
		y_y = create_draw(y, y, batch_size)
		y_yy = create_draw(y, yy, batch_size)
		y_xy = create_draw(y, xy, batch_size)

		images = torch.cat([x  , xx , x   , x   ], dim=2)
		masks  = torch.cat([y_y, y_y, y_yy, y_xy], dim=2)
		mask_caption = ''
		...
		for img, mask in zip(images, masks):
			wdb_img = wandb.Image(img, caption=caption,
											masks={mask_caption: {'mask_data': mask, 'class_labels': class_labels}})
			list_wdb_imgs.append(wdb_img)
