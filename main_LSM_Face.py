...
from datasets.transforms import face_transforms_source, face_transforms_target
from src.Callbacks.BestModel import BestModel
from src.Model_LSM_General import PL_LSM_CNN_MLP, LSM_Lambdas
from src.Callbacks.SampleLogger.SampleLogger_IODA_FACE import SampleLogger_Face_IODA

from params.Face.model_params_face_lsm import cfg
from params.slurm_params import cfg_slurm
from params.Face.params_face_64 import cfg_dataset
...
def main(params: dict):
	...
	data: SOP_data = get_dataloader_SOP(
		...
		shuffle_mode=True,
		transforms_source=face_transforms_source,
		transforms_target=face_transforms_target,
	...
	callbacks = [
		BestModel(
			monitor="valid/metric/nrmse_xy",
			mode='min',
		),
		SampleLogger_Face_IODA(
	...
	if params['use_early_stop']:
		early_stop = EarlyStopping(monitor=f"valid/metric/nrmse_xy", min_delta=params['delta_min'],
		                           patience=params['patience'], verbose=True, mode='min', strict=True)
	...
	return wandb.summary['test/metric/nrmse_xy']
